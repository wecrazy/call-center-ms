...
var getDataBasedOnWONumberMutex sync.Mutex

func (h *WhatsmeowHandler) GetDataBasedOnWONumber(v *events.Message, stanzaID, originalSenderJID string) error {
	if !getDataBasedOnWONumberMutex.TryLock() {
		return fmt.Errorf("%v", "getDataBasedOnWONumber is already running, skipping execution.")
	}
	defer getDataBasedOnWONumberMutex.Unlock()

	taskDoing := "WO Number Need Follow Up by Call Center via Excel in WAG"

	// nil value
	var dateTimeFormatPlanSchedule *time.Time = nil
	orderWish := "Merchant Confirmation"
	requestType := "WO Number Need to Follow Up by Call Center Team via Excel File in WAG"
	lastUpdateBy := "System"
	requestToCC := "Konfirmasi ke PIC merchant terkait kondisi EDC, apakah sudah terpasang / belum, perangkat EDC dalam kondisi yang baik atau tidak. Perhatikan remark / description yang ada, serta konfirmasikan terkait jam operasional di merchant beserta kemungkinan kunjugan teknisi yang tidak bisa ke merchant karena ada kendala baik itu di jarak, lokasi dan lain."

	// Running scheduler get data based on wo number
	...
	// 	5, // "Tes Dev Mfjr",
	// }
	var woNumberIncluded []string
	if err := h.Database.Model(&models.BasedOnWoNumber{}).Pluck("wo_number", &woNumberIncluded).Error; err != nil {
		return fmt.Errorf("failed to retrieve wo number: %v", err)
	}
	...
	domain := []interface{}{
		...
		[]interface{}{"x_no_task", "=", woNumberIncluded},
	...
	if !ok {
		return fmt.Errorf("%v", "error: failed to assert results as []interface{}")
	}
	...
	if totalDataGet == 0 && totalDataBeingInsert == 0 && totalDataBeingUpdate == 0 && totalDataCannotBeFU == 0 {
		...
		sb.WriteString(fmt.Sprintf("⚠ Kami mohon maaf, karena tidak adanya data %v yang dapat di-follow up oleh tim *Call Center* pada hari ini, karena terdapat masalah pada _system_.\n", taskDoing))
	...
	var sb strings.Builder
	sb.WriteString(fmt.Sprintf("*[INFO]* Berikut hasil tarikan _system_ terkait data %v - %v untuk selanjutnya di-follow up oleh tim *Call Center*:\n", taskDoing, requestType))
	sb.WriteString(fmt.Sprintf(
		"\nTotal Data Tarikan %v: %d\nTotal Data Baru yang Diinput ke Database: %d\nTotal Data yang Diupdate di Database: %d\nTotal Data Invalid / Tidak Bisa di Follow Up: %d",
		taskDoing, totalDataGet, totalDataBeingInsert, totalDataBeingUpdate, totalDataCannotBeFU,
	))
	msgToSend := sb.String()
	quotedMsg := &waProto.ContextInfo{
		StanzaID:      &stanzaID,
	...
	if totalDataCannotBeFU > 0 {
		// var sb strings.Builder
		// sb.WriteString(fmt.Sprintf("⚠ Kami mohon maaf, karena %d data %v - %v berikut tidak dapat di-follow up oleh tim *Call Center*:\n", totalDataCannotBeFU, taskDoing, requestType))
