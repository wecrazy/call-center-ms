...
var getDataSLAHmin2Mutex sync.Mutex

func (h *WhatsmeowHandler) GetDataSLAHmin2forFU() {
	if !getDataSLAHmin2Mutex.TryLock() {
		log.Println("GetDataSLAHmin2forFU is already running, skipping execution.")
		return
	}
	defer getDataSLAHmin2Mutex.Unlock()
	...
	log.Printf("Running scheduler Get Data SLA H-2 for Followed Up by CC @%v", time.Now())

	// Running scheduler get data SLA H-2
	orderWish := "Merchant Confirmation"
	requestType := "SLA H-2 Follow Up"
	lastUpdateBy := "System"
	requestToCC := "Konfirmasi ke PIC merchant kapan teknisi bisa melakukan kunjungan"
	...
	startParam := now.Truncate(24 * time.Hour).Add(-7 * time.Hour)
	endParam := startParam.Add(2*24*time.Hour + 23*time.Hour + 59*time.Minute + 59*time.Second)
	...
	domain := []interface{}{
		...
		[]interface{}{"x_sla_deadline", ">=", startParam},
		[]interface{}{"x_sla_deadline", "<=", endParam},
	...
	for i, record := range resultArray {
		...
		err = h.Database.Table(h.YamlCfg.Db.TbWaReq).
			// Where("id = ? AND is_done = ? AND temp_cs = 0",
			// 	dataToDB.ID,
			// 	false,
			// ).
	...
	totalDataSLAHmin2 = len(resultArray)
	...
	if totalDataSLAHmin2 == 0 && totalDataBeingInsert == 0 && totalDataBeingUpdate == 0 && totalDataCannotBeFU == 0 {
		var sb strings.Builder
		sb.WriteString("⚠ Kami mohon maaf, karena tidak adanya data SLA H-2 yang dapat di-follow up oleh tim *Call Center* pada hari ini, karena terdapat masalah pada _system_.\n")
	...
	var sb strings.Builder
	sb.WriteString("*[INFO]* Berikut hasil tarikan _system_ terkait data SLA H-2 untuk selanjutnya di-follow up oleh tim *Call Center*:\n")
	sb.WriteString(fmt.Sprintf(
		"\nTotal Data Tarikan SLA H-2: %d\nTotal Data Baru yang Diinput ke Database: %d\nTotal Data yang Diupdate di Database: %d\nTotal Data Invalid / Tidak Bisa di Follow Up: %d",
		totalDataSLAHmin2, totalDataBeingInsert, totalDataBeingUpdate, totalDataCannotBeFU,
	))
	msgToSend := sb.String()
	_, err = h.Client.SendMessage(context.Background(), jid, &waProto.Message{
		ExtendedTextMessage: &waProto.ExtendedTextMessage{
	...
	if totalDataCannotBeFU > 0 {
		// var sb strings.Builder
		// sb.WriteString(fmt.Sprintf("⚠ Kami mohon maaf, karena %d data SLA H-2 berikut tidak dapat di-follow up oleh tim *Call Center*:\n", totalDataCannotBeFU))
	...
	log.Printf("Scheduler Get SLA Data H-2 for FU by Call Center successfully executed @%v", time.Now())
}
